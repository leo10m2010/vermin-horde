...
test('renders a nonblank interactive game canvas', async ({ page }, testInfo) => {
  ...
  // The game boots into a main menu (phase 'menu'); enter the run via the
  // deterministic test hook before exercising movement.
  await page.evaluate(() => {
    window.__THREE_GAME_TEST_HOOKS__?.seed(1);
